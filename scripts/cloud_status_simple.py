...
def generate_team_password(team_id: int) -> str:
    if config.CONFIG_FILE:
        folder = str(Path(config.CONFIG_FILE).parent.name)  # for example "2020" or "test"
    else:
        folder = 'none'
...
def simple_cloud_status(kinds: List[str], reset: bool, running: bool) -> None:
    if len(kinds) == 0:
    ...
    if not reset:
        try:
            ...
            print(f'[!] Could not load {config.CLOUDCONFIG_FILE}')
