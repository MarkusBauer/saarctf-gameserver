...
from saarctf_commons.config import CHECKER_PACKAGES_PATH, CHECKER_PACKAGES_LFS

sys.path.append(CHECKER_PACKAGES_PATH)


class PackageLoader:
	os.makedirs(CHECKER_PACKAGES_PATH, exist_ok=True)
	lock = FileLock(CHECKER_PACKAGES_PATH + '/.lock')
	...
	def package_exists(cls, package: str) -> bool:
		...
		"""
		return os.path.exists(CHECKER_PACKAGES_PATH + '/' + package)
	...
	def ensure_package_exists(cls, package: str) -> bool:
		...
		with cls.lock:
			...
			if cls.package_exists(package):
				return False
			DBFilesystem().load_package_to_folder(package, CHECKER_PACKAGES_PATH + '/' + package, CHECKER_PACKAGES_LFS)
