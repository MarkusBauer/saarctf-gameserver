...
from controlserver.models import CheckerFilesystem, db, CheckerFile


class DBFilesystem:
	ignore_patterns = [re.compile(r'^__pycache__$'), re.compile(r'\.pyc$'), re.compile(r'^\.idea$'), re.compile(r'^\.git'), re.compile(r'^\.mypy_cache$'), re.compile(r'^gamelib$')]
	...
	def store_file_in_database(self, fname: str) -> str:
		...
		if CheckerFile.query.filter(CheckerFile.file_hash == hash).count() == 0:
			db.session.add(CheckerFile(file_hash=hash, content=data))
			db.session.commit()
			print('Stored {} as {} in db'.format(fname, hash))
		return hash

	def load_package_to_folder(self, package: str, folder: str, lfs_path: str = None) -> bool:
		"""
		...
		filesystem: List[CheckerFilesystem] = CheckerFilesystem.query.filter(CheckerFilesystem.package == package).order_by(
			CheckerFilesystem.path).all()
		for fs in filesystem:
			if fs.file_hash:
				...
				if lfs_path and os.path.exists(lfs_path + '/' + fs.file_hash):
					...
					file: CheckerFile = CheckerFile.query.filter(CheckerFile.file_hash == fs.file_hash).first()
