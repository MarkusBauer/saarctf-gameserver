...
from controlserver.models import LogMessage, db_session
...
def log_messages_index(page: int = 1) -> ResponseReturnValue:
    per_page = 50
    ...
    log_messages: Pagination = paginate_query(query, page, per_page)
    if log_messages.pages < page:
        page = log_messages.pages if log_messages.pages > 0 else 1
        log_messages = paginate_query(query, page, per_page)

    session = db_session()
    components = [(c, c) for c, in session.query(LogMessage.component.distinct()).order_by(LogMessage.component).all()]
...
def log_messages_view(id: int | None = None) -> ResponseReturnValue:
