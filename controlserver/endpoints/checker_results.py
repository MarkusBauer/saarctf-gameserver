...
from controlserver.models import CheckerResult, Service, Team, db
from saarctf_commons.config import FLOWER_URL
...
def checker_results_index(page=1):
	...
	checker_results: Pagination = query.paginate(page, per_page, error_out=False)
	if checker_results.pages < page:
		page = checker_results.pages if checker_results.pages > 0 else 1
		checker_results = query.paginate(page, per_page, error_out=False)
	...
	return render_template(
		...
		FLOWER_URL=FLOWER_URL, states=CheckerResult.states, query_string=request.query_string.decode('utf8'),
...
def checker_results_view(id=None):
	...
	return render_template('checker_results_view.html', checker_result=checker_result, FLOWER_URL=FLOWER_URL)
