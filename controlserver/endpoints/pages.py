...
def frontpage() -> ResponseReturnValue:
    ...
    scoreboard_paths = (config.SCOREBOARD_PATH, config.SCOREBOARD_PATH_INTERNAL)
    missing_scoreboard = not all(
        (p / "api" / "scoreboard_current.json").exists() for p in scoreboard_paths if p
    )
    unwriteable_scoreboard = not missing_scoreboard and all(
        not os.access(p, os.W_OK | os.X_OK)
        or not os.access(p / "api", os.W_OK | os.X_OK)
        or ((p / "index.css").exists() and not os.access(p / "index.css", os.W_OK))
        for p in scoreboard_paths if p
    )
    ...
    return render_template(
        ...
        missing_scripts=", ".join(
            [service_name for (service_name,) in missing_scripts]
        ),
...
def overview_set_timing() -> ResponseReturnValue:
    ...
    if "openVulnboxAccessAt" in body:
        openVulnboxAccessAt = body["openVulnboxAccessAt"] or None
        if openVulnboxAccessAt is None or openVulnboxAccessAt >= int(time.time()):
            Timer.open_vulnbox_access_at = openVulnboxAccessAt
    return "OK"
...
def overview_vpn() -> ResponseReturnValue:
    ...
    return jsonify(
        {
            "state": vpn.get_state().value,
            "banned": banned_teams,
            "permissions": open_teams,
            "teams_online": teams_online,
            "teams_online_once": teams_online_once,
            "teams_offline": teams_offline,
            "traffic_stats": trafficstats[::-1],
            "traffic_stats_keys": trafficstats_keys[::-1],
            "openVulnboxAccessAt": Timer.open_vulnbox_access_at,
            "serverTime": int(time.time()),
        }
    )
...
def packages() -> ResponseReturnValue:
    ...
    return render_template(
        ...
        file_size=round((file_size or 0) / 1024),
    ...
    if service.checker_script_dir:
        if not os.path.exists(service.checker_script_dir):
            ...
            if package == service.package:
                ...
                message = 'NEW script "{}"'.format(package[:12])
            else:
                message = 'OLD script "{}"'.format(package[:12])
    else:
        message = "not managed by package"
        package = None

    from controlserver.dispatcher import DispatcherFactory

    dispatcher = DispatcherFactory.build(config.RUNNER.dispatcher)
    ref, result = dispatcher.dispatch_test_script(team, service, tick, package)
    return jsonify(
        {
            "task": ref,
            "result_id": result.id,
...
def checker_status(tick: int | None = None) -> ResponseReturnValue:
    ...
    dispatcher = DispatcherFactory.build(config.RUNNER.dispatcher)
    combinations = dispatcher.get_tick_combinations(tick)
    if not combinations:
    ...
    return render_template(
        ...
        count_finished=count or 0,
        count_dispatched=len(combinations),
        server_time=int(time.time()),
        tick_start=tick_start,
        tick_start_dt=datetime.datetime.fromtimestamp(tick_start),
        tick_end=tick_end,
        stats_dispatched=stats_dispatched,
        stats_finished=stats_finished,
        stats_time=stats_time,
        stats_status=stats_status,
        stats_time_count=stats_time_count,
        stats_toolate=stats_toolate,
        total_toolate=sum(stats_toolate.values()),
        ...
        finished_per_tick=finished_per_tick,
        no_finished_timestamp=no_finished_timestamp,
...
def checker_status_overview() -> ResponseReturnValue:
    ...
    for i in range(first_tick, Timer.current_tick + 1):
        ticks[i] = {
            ...
            'dispatched': len(DispatcherFactory.build(config.RUNNER.dispatcher).get_tick_combinations(i) or []),
            'tasks_ok': 0,
            'tasks_warn': 0,
            'tasks_error': 0,
            'tasks_revoked': 0,
...
def recreate_scoreboard() -> ResponseReturnValue:
    def recreate_scoreboard_inner() -> None:
        ...
        for i, scoreboard in enumerate(default_scoreboards(scoring), start=1):
            ...
            while rn <= end_round:
                scoreboard.create_scoreboard(
                    rn, Timer.state != CTFState.STOPPED or end_round > 0, rn == end_round
                )
                rn += 1
                end_round = (
                    Timer.current_tick - 1
    ...
    threading.Thread(
        target=do_scoreboard, name="manual_scoreboard", daemon=True
    ).start()
    return jsonify({"ok": True, "message": "Recalculation started"})


@app.route("/scripts/service_update", methods=["POST"])
def service_update() -> ResponseReturnValue:
    service_mgr = ServiceRepoManager()
    try:
        service_mgr.update_all_services()
        service_mgr.messages.append("OK")
        return jsonify(service_mgr.messages)
    except Exception as e:
        service_mgr.messages.append(f"Error: {e}")
        service_mgr.messages.append(traceback.format_exc())
        return jsonify(service_mgr.messages), 500
