...
from controlserver.models import db, TeamPoints, Team, Service, SubmittedFlag, TeamRanking, CheckerResultLite
from controlserver.scoring.scoring import ScoringCalculation
from controlserver.app import app
from sample_files.debug_sql_timing import timing, print_query_stats, reset_timing

assert config.CONFIG_FILE.endswith('/config.test.json')
config.FLAG_ROUNDS_VALID = 10
config.CONFIG['nop_team_id'] = -1
config.NOP_TEAM_ID = -1
...
class MyTestCase(unittest.TestCase):
	...
	def _init_teams(self, team_count: int, **kwargs):
		for i in range(team_count):
			db.session.add(Team(name=f'Team {i + 1}', vpn_connected=True, **kwargs))

	def _init_services(self, service_count: int, **kwargs):
		for i in range(service_count):
			db.session.add(Service(name=f'Service {i + 1}', checker_script='', **kwargs))
