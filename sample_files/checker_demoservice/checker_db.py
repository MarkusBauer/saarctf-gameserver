...
class DBService(gamelib.ServiceInterface):
    ...
    def store_flags(self, team, round):
        ...
        cursor.execute(
            "INSERT INTO test_service_storage (team_id, service_id, round, flag) "
            "VALUES (%s, %s, %s, %s), (%s, %s, %s, %s), (%s, %s, %s, %s)", (
                team.id, self.id, round, self.get_flag(team, round, 0),
                team.id, self.id, round, self.get_flag(team, round, 1),
                team.id, self.id, round, self.get_flag(team, round, 2)
            )
        )
        conn.commit()
        cursor.close()
    ...
    def retrieve_flags(self, team, round):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT flag FROM test_service_storage WHERE team_id = %s AND service_id = %s AND round = %s",
            (team.id, self.id, round)
        )
